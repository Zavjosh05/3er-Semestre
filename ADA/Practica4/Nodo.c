...
//  crearArista(node->aristas);
//  *(node->aristas) = (Arista)calloc(1, sizeof(t_Arista));
...
int numeroDeNodos(Nodo node, Nodo *arrNodo, int numNodos, Arista *arrArista, int numArista)
{
  if(node == NULL || arrNodo == NULL || arrArista == NULL)
    return 0;
